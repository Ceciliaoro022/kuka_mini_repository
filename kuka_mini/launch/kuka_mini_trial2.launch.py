...
def generate_launch_description():
    ...
    #I do not think I need to change it because the yaml file already have the controller for both
    ...
    kuka_controller_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=[kuka_controller, '--controller-manager', [namespace, 'controller_manager']],
    )

    mini_controller_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=[mini_controller, '--controller-manager', [namespace, 'controller_manager']],
    )
    ...
    delay_kuka_controller_spawner_after_joint_state_broadcaster_spawner = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=joint_state_broadcaster_spawner,
            on_exit=[kuka_controller_spawner],
        )
    )

    #I added this, position matters. It has to be after joint_state_broadcaster
    delay_mini_controller_spawner_after_joint_state_broadcaster_spawner = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=joint_state_broadcaster_spawner,
            on_exit=[mini_controller_spawner],
        )
    )
    ...
    nodes = [
        ...
        delay_kuka_controller_spawner_after_joint_state_broadcaster_spawner,
        delay_mini_controller_spawner_after_joint_state_broadcaster_spawner,
    ]
